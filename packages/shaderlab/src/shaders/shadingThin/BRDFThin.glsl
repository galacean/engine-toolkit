#ifndef BRDF_THIN_INCLUDED
#define BRDF_THIN_INCLUDED 1
...
float Depol (vec3 polV) {return 0.5 * (polV.x + polV.y); }
vec3 DepolColor (vec3 colS, vec3 colP){ return 0.5 * (colS + colP); }

vec3 Fresnel0ToIor(vec3 F0)
{
  vec3 sqrtF0 = sqrt(F0);
  return (vec3(1.0,1.0,1.0) + sqrtF0) / (vec3(1.0,1.0,1.0) - sqrtF0);
}
...
{ 
    ...
    // float eta_2 = mix(2.0, 1.0, iridescenceThickness);
     ...
     //if (cosTheta2Sq < 0.0) {return vec3(1.0,1.0,1.0);}
    ...
    // if (eta2 < eta1) { phi12 = PI;}
     ...
     // Phase shift
	   float OPD = dinc * cosTheta1 * eta2;//dinc * cosTheta2;
	   vec3 phi = vec3(phi21,phi21,phi21) + phi23;

	   vec3 R123 = clamp(R12 * R23, 1e-5, 0.9999);
	   vec3 r123 = sqrt(R123);
     vec3 Rs   = sqr(T121) * R23 / (vec3(1.0,1.0,1.0)-R123);
     ...
     for (int m = 1; m <= 2; ++m)
        {
         Cm *= r123;
		     ...
		     I += Cm * Sm;//DepolColor(Cm.x * SmS, Cm.y * SmP);
        }
...
vec3 DirectBDRFIridescence(vec3 incidentDirection, vec3 viewDir, vec3 normal,vec3 specularColor, float roughness )
{
    // Compute dot products
    float NdotL = saturate( dot( normal, incidentDirection ) );
    float NdotV = saturate( dot( normal, viewDir ) );
    vec3 halfDir = normalize( incidentDirection + viewDir );
    float NdotH = saturate( dot( normal, halfDir ) );
   
    float cosTheta1 = dot(halfDir, incidentDirection);
    ...
    float G = G_GGX_SmithCorrelated(roughness, NdotL, NdotV);

    vec3 color = I ; // (4.0 * NdotL * NdotV);
