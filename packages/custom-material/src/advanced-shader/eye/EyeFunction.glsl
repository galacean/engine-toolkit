...
vec4 calculateEyeColor(vec2 uv, mat3 tbn, SurfaceData surfaceData)
{
  //Sclera UV
  vec2 scleraUV = (uv * material_ScleraSize)-((material_ScleraSize-1.0)/2.0);
...
vec3 calculateEyeNormal(vec2 uv, mat3 tbn, bool isFrontFacing)
{
  //Sclera UV
  vec2 scleraUV = (uv * material_ScleraSize)-((material_ScleraSize-1.0)/2.0);
  //Iris UV
