...
#if defined(SHOW_NORMAL) && defined(GALACEAN_HAS_NORMAL)
    if (gl_VertexID % 2 == 1) {
...
#if defined(SHOW_TANGENT) && defined(GALACEAN_HAS_TANGENT)
    if (gl_VertexID % 2 == 1) {
...
#if defined(SHOW_BITANGENT) && defined(GALACEAN_HAS_TANGENT) && defined(GALACEAN_HAS_NORMAL)
    if (gl_VertexID % 2 == 1) {
        vec3 normalW = normalize( mat3(u_worldNormal) * normal.xyz );
        vec3 tangentW = normalize( mat3(u_worldNormal) * tangent.xyz );
